...
// Funkce per role: ZDARMA vs HUBplus (zobrazí se po kliknutí na kartu).
...
export function ServiceMap({ showMap = true, showCards = true, hideKeys = [] }: { showMap?: boolean; showCards?: boolean; hideKeys?: string[] }) {
  ...
  return (
    <div className="smap-wrap">
      ...
      {/* MENU vybrané služby (Zdarma vs HUBplus) */}
      {selSvc && funcs && (
        <div className="hive-menu" style={{ borderTopColor: selSvc.color }}>
          ...
          <div className="hive-menu-grid">
            {funcs.member.map((f) =>
              f.href ? (
                <Link key={f.label} href={f.href} className="hm-item hm-member">{f.label} <span className="hm-badge">HUBplus</span></Link>
              ) : (
                <span key={f.label} className="hm-item hm-member hm-disabled">{f.label} <span className="hm-badge">HUBplus</span></span>
              )
            )}
          </div>
        </div>
