...
// Jeden ODDÍL profilu podle role (trenér/vyplétač/fyzio/fitness = specialist; areál = venue).
// Self-contained: načte a ukládá si vlastní data. Profil jen vyskládá oddíly pod sebe dle zaškrtnutých rolí.
// „Veřejná karta" NENÍ extra entita — tyhle oddíly JSOU to, co cizí vidí na veřejném profilu.
...
type Venue = {
  ...
  photo_url: string | null; status: string | null; verified: boolean | null; verify_requested: boolean | null;
};
...
const WEEKDAYS: [number, string][] = [[1, "Pondělí"], [2, "Úterý"], [3, "Středa"], [4, "Čtvrtek"], [5, "Pátek"], [6, "Sobota"], [0, "Neděle"]];
...
export default function RoleSection({ userId, role, identity, canPro }: { userId: string; role: string; identity: Identity; canPro: boolean }) {
  ...
  const load = useCallback(async () => {
    ...
    if (sp) {
      const [{ data: svc }, { data: av }] = await Promise.all([
        sb.from("services").select("id,name,price_czk,duration_min").eq("specialist_id", sp.id),
        sb.from("availability").select("weekday,start_min,end_min,slot_min").eq("specialist_id", sp.id),
      ]);
  ...
  const saveSpec = async () => {
    ...
    const rows = services.filter((s) => s.name.trim()).map((s) => ({ specialist_id: spec.id, name: s.name.trim(), price_czk: Number(s.price_czk) || 0, duration_min: s.duration_min ? Number(s.duration_min) : null }));
    if (rows.length) await sb.from("services").insert(rows);
    await sb.from("availability").delete().eq("specialist_id", spec.id);
    const aRows = avail.filter((a) => a.from && a.to && strToMin(a.to) > strToMin(a.from)).map((a) => ({ specialist_id: spec.id, weekday: a.weekday, start_min: strToMin(a.from), end_min: strToMin(a.to), slot_min: a.slot || 60 }));
  ...
  const saveVenue = async () => {
    ...
    setBusy(true);
    const sb = createClient();
    await sb.from("venues").update({ name: venue.name, city: venue.city, description: venue.description, website: venue.website, reservation_url: venue.reservation_url, amenities: venue.amenities }).eq("id", venue.id);
    setBusy(false); flash("Uloženo ✓");
  };

  const VerifyBadge = ({ verified }: { verified: boolean | null }) =>
    verified ? <span className="member-badge"><BadgeCheck size={14} style={{ verticalAlign: "-2px" }} /> Ověřeno</span> : <span className="nomember">neověřeno</span>;

  if (loading) return <div className="acct-card"><p className="member-note">Načítám oddíl {ROLE_LABEL[role]}…</p></div>;

  // ── AREÁL ──
  if (isVenue) {
    return (
      <div className="acct-card">
        <div className="acct-card-head"><Building2 size={20} /><h2>{ROLE_LABEL[role]}</h2>{venue && <VerifyBadge verified={venue.verified} />}</div>
        {!venue ? (
  ...
  return (
    <div className="acct-card">
      <div className="acct-card-head"><UserCog size={20} /><h2>{ROLE_LABEL[role]}</h2>{spec && <VerifyBadge verified={spec.verified} />}</div>
      {!spec ? (
        <div className="card-create">
          <p className="member-note">Zatím nemáš vyplněný oddíl <b>{ROLE_LABEL[role]}</b>. Vytvoř ho a objevíš se na mapě i v katalogu.</p>
          <button className="btn btn-green" disabled={busy} onClick={createSpecialist}><UserCog size={15} /> Vytvořit oddíl ({ROLE_LABEL[role]})</button>
        </div>
      ) : (<>
        <div className="card-identity">
          <div className="card-photo-prev sm" style={identity.photoUrl ? { backgroundImage: `url(${identity.photoUrl})` } : undefined}>{!identity.photoUrl && <UserCog size={22} />}</div>
          ...
          <div className="cenik">
            ...
            {avail.map((a, i) => (
              <div className="avail-row" key={i}>
                <select value={a.weekday} onChange={(e) => setAvail(avail.map((x, j) => j === i ? { ...x, weekday: Number(e.target.value) } : x))}>{WEEKDAYS.map(([w, l]) => <option key={w} value={w}>{l}</option>)}</select>
                ...
                <select value={a.slot} onChange={(e) => setAvail(avail.map((x, j) => j === i ? { ...x, slot: Number(e.target.value) } : x))}>{[30, 45, 60, 90].map((s) => <option key={s} value={s}>{s} min</option>)}</select>
